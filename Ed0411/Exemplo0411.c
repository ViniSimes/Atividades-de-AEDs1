...
/**
  Funcao para determinar sa valor inteiro e' positivo
...
void method02 ( )
{
  ...
  //repetir para a quatidade de vezes informada
  controle = 1;
  while ( controle <= quantidade)
  {
    //ler valor do teclado
  ...
  //mostrar a quantidade de valores psitivos
  IO_printf ( "%s%d\n", "Positivos = ", contador );
...
void method03 ( )
{
  ...
  IO_printf ( "%s%d\n", "Positivismo menores que 100 = ", contador );  
...
bool even ( int x )
{
  ...
  //testar a condnicao ( resto inteiro (%) da divisao por 2 igual a zero )
  if ( x % 2 == 0 )
  {
    result = true;
  }// end if
